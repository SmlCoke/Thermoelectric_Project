...
def test_model(config, num_samples=5):
    ...
    # 为每个通道单独计算指标
    channel_names = ['VSW_black', 'VSW_red', 'VSW_blue', 'VSW_green', 
...
def visualize_predictions(inputs, predictions, targets, channel_names, config, num_samples=5):
    ...
    for idx, sample_idx in enumerate(sample_indices):
        ...
        fig.suptitle(f'Test example {sample_idx + 1} - Prediction results', fontsize=16)
        ...
        for ch_idx in range(8):
            ...
            ax.plot(input_hours, input_seq[:, ch_idx], 'b-', 
                   label='History data', linewidth=1, alpha=0.7)
            
            # practical value (绿色)
            ax.plot(output_hours, target_seq[:, ch_idx], 'g-', 
                   label='Practical value', linewidth=2)
            
            # 预测值（红色虚线）
            ax.plot(output_hours, pred_seq[:, ch_idx], 'r--', 
                   label='Prediction value', linewidth=2)
    ...
    fig.suptitle('Prediction error distribution for each channel', fontsize=16)
    ...
    # 绘制VSW黑和VLW黑（最能体现云遮挡）
    ax.plot(output_hours, target_seq[:, 0], 'b-', label='VSW_black(Practical)', linewidth=2)
    ax.plot(output_hours, pred_seq[:, 0], 'b--', label='VSW_black(Prediction)', linewidth=2)
    ax.plot(output_hours, target_seq[:, 4], 'r-', label='VLW_black(Practical)', linewidth=2)
    ax.plot(output_hours, pred_seq[:, 4], 'r--', label='VLW_black(Prediction)', linewidth=2)

    ax.set_xlabel('Prediction time (hours)', fontsize=12)
    ax.set_ylabel('Normalized Voltage', fontsize=12)
    ax.set_title('Cloud Cover Event Prediction Example\n(VSW down + VLW up = Cloud is coming)', fontsize=14)
    ax.legend(loc='best')
    ax.grid(True, alpha=0.3)
