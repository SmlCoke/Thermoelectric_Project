...
def visualize_data(data, time_hours, save_dir='.'):
    ...
    ax.plot(time_slice, data[start_idx:end_idx, 0], label='VSW黑', linewidth=2)
    ax.plot(time_slice, data[start_idx:end_idx, 4], label='VLW黑', linewidth=2)
    ax.set_xlabel('时间 (小时)')
    ax.set_ylabel('电压 (归一化)')
    ax.set_title('云遮挡事件示例：VSW下降，VLW上升')
    ax.legend()
    ax.grid(True, alpha=0.3)
