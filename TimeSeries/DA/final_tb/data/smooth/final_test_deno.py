...
def find_files_in_folder(folder_path):
    ...
    denoise_pattern = re.compile(r"data\d+_\d?_L(\d+)\.csv$")
...
def plot_multiple(data_entries, start_ratio=0.0, end_ratio=1.0, target_channel=None, save_path=None, show_diff=False,
    ...
    if target_channel:
        if target_channel not in slices[0]["data"].columns:
            ...
            return
        # ---- 新布局：单通道且存在降噪数据时，每个降噪文件单独一列，含主图+残差 ----
        if len(slices) > 1:
            denoise_slices = slices[1:]  # 跳过原始
            n = len(denoise_slices)
            fig, axes = plt.subplots(2, n, figsize=(4.5*n, 6), sharex=True, gridspec_kw={'height_ratios': [3, 1]})
            if n == 1:
            ...
            for idx, slice_item in enumerate(denoise_slices):
                ...
                ax_main.plot(x_axis, slice_item["data"][target_channel], color=slice_item["color"], linewidth=1, label=slice_item["label"])
                ax_main.set_title(f'{target_channel} (L={slice_item["label"].split("=")[-1]})', fontproperties=T_14)
                ...
                # 残差
                ax_diff.plot(x_axis, slice_item["data"][target_channel] - raw_series,
