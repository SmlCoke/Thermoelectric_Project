...
def find_files_in_folder(folder_path):
    ...
    denoise_pattern = re.compile(r"data\d+_\d?_denoised\.csv$")
    ...
    for f in files:
        ...
        if match:
            threshold = 1.75
            L = 5
            denoise_files.append((threshold, L, f))
    ...
    denoise_paths = [(thr, L, os.path.join(folder_path, f)) for thr, L, f in denoise_files]
...
def plot_multiple(data_entries, start_ratio=0.0, end_ratio=1.0, target_channel=None, save_path=None, show_diff=False,
    ...
    if target_channel:
        ...
        if len(slices) > 1:
            ...
            fig, axes = plt.subplots(2, n, figsize=(9, 6),
...
if __name__ == "__main__":
    ...
    if args.folder:
        ...
        for idx, (thr, L, path) in enumerate(denoise_paths):
            df = pd.read_csv(path)
            color = cmap(idx % cmap.N)
            data_entries.append({
                "df": df,
                "label": f"Denoised n={thr} L={L}",
                "color": color,
            })
