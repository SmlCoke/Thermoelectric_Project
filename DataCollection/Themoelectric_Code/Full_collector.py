...
# START_TIME 和 DATA_FILE 的定义从这里移除
import board
import busio
...
def main():
    ...
    data_file = f"TEC_data_{start_time.strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    print(f"数据文件: {data_file}") # 使用局部变量 data_file
    ...
    print()
    # 初始化CSV文件
    initialize_csv_file(data_file) # 使用局部变量 data_file
    ...
    try:
        ...
        while True:
            ...
            if any(v is not None for v in voltages):
                save_data(data_file, timestamp, datetime_str, voltages) # 使用局部变量 data_file
                sample_count += 1
        ...
        print(f"数据已保存至: {data_file}") # 使用局部变量 data_file
        print("=" * 80)
    except Exception as e:
        print(f"\n发生错误: {e}")
        print("程序异常退出")
